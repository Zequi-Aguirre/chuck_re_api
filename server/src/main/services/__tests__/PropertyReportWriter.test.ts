...
describe("PropertyReportWriter (JAK-130/131)", () => {
    ...
    describe("Next Commands menu removal (JAK-kill-report-menu)", () => {
        it("forbids the command menu in the hard guardrails", () => {
            const { writer } = makeWriter(DEFAULT_STYLE);
            const system = writer.composeSystemPrompt(DEFAULT_STYLE);
            expect(system).toMatch(/Next Commands/i);
            expect(system).toMatch(/no upsell|call-to-action|Do NOT append a menu/i);
        });

        it("strips a Next Commands menu the model appends before the footer; body + footer survive", async () => {
            const { writer, llm } = makeWriter(DEFAULT_STYLE);
            llm.generateText.mockResolvedValue(
                [
                    "Jake Property Report",
            ...
            expect(out).not.toMatch(/Next Commands/i);
            expect(out).not.toMatch(/Text SKIP/i);
            expect(out).not.toMatch(/Text COMP/i);
            // The report body and the canonical footer are untouched.
            expect(out).toContain("742 Evergreen Terrace");
            expect(out.endsWith("Every Lead Deserves Jake.\nGoTextJake.com/CRM")).toBe(true);
        });

        it("strips the menu even when it sits directly on top of the footer (no blank line)", async () => {
            const { writer, llm } = makeWriter(DEFAULT_STYLE);
            llm.generateText.mockResolvedValue(
                [
                    "742 Evergreen Terrace",
            ...
            expect(out).not.toMatch(/Next Commands|Text SKIP|Text COMP/i);
            expect(out).toContain("742 Evergreen Terrace");
            expect(out.endsWith("Every Lead Deserves Jake.\nGoTextJake.com/CRM")).toBe(true);
        });

        it("static stripCommandMenu removes stray advertised command lines without a header", () => {
            const cleaned = PropertyReportWriter.stripCommandMenu(
                "Owner: Homer Simpson\nText SKIP for owner contact\nReply COMP for comparables\nDone."
            );
            expect(cleaned).toBe("Owner: Homer Simpson\nDone.");
        });

        it("static stripCommandMenu leaves a normal report untouched", () => {
            const report = "Jake Property Report\n\n742 Evergreen Terrace\n\nOwnership\n- Homer Simpson";
            expect(PropertyReportWriter.stripCommandMenu(report)).toBe(report);
        });
    });
